...
coretest.describe_mapper_test_case({
    ...
    const frags = _.concat(
        _.times(data_frags, data_index => ({ _id: new ObjectId(), data_index })),
        _.times(parity_frags, parity_index => ({ _id: new ObjectId(), parity_index }))
    );
    const first_pools = _.times(num_pools, i => ({ _id: new ObjectId(), name: 'first_pool' + i, }));
    const second_pools = _.times(num_pools, i => ({ _id: new ObjectId(), name: 'second_pool' + i, }));
    const external_pools = _.times(num_pools, i => ({ _id: new ObjectId(), name: 'external_pool' + i, }));
