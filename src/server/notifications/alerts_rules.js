...
function only_once(sev, sysid, alert) {
    return Promise.resolve(AlertsLogStore.instance().find_alert(sev, sysid, alert))
        .then(res => res.length === 0);
}

function only_once_by_regex(regex) {
    return function(sev, sysid, alert) {
        return Promise.resolve(AlertsLogStore.instance().find_alert(sev, sysid, { $regex: regex }))
            .then(res => (res.length === 0));
    };
}
...
function once_every(interval) {
    return function(sev, sysid, alert) {
        ...
        return Promise.resolve(AlertsLogStore.instance().find_alert(sev, sysid, alert, { $gt: interval_date }))
            .then(res => (res.length === 0));
    };
}
...
function once_daily(sev, sysid, alert) {
    ...
    return Promise.resolve(AlertsLogStore.instance().find_alert(sev, sysid, alert, { $gt: one_day_ago }))
        .then(res => (res.length === 0));
}
...
function once_weekly(sev, sysid, alert) {
    ...
    return Promise.resolve(AlertsLogStore.instance().find_alert(sev, sysid, alert, { $gt: one_week_ago }))
        .then(res => (res.length === 0));
}
