...
class BucketDiff {
     ...
     * }} params
     */
    constructor({ first_bucket, second_bucket, s3_params, for_replication }) {
    ...
    async get_objects(bucket, prefix, max_keys, version, curr_bucket_cont_token) {
        dbg.log2('BucketDiff get_objects::', bucket, prefix, max_keys, version, curr_bucket_cont_token);
    ...
    async get_keys_diff(first_bucket_keys, second_bucket_keys, second_bucket_cont_token, version) {
        ...
        this._process_keys_in_range(ans, second_bucket_keys, second_bucket_cont_token, version);
     ...
     * @param {boolean} version
     */
    _process_keys_in_range(ans, second_bucket_keys, second_bucket_cont_token, version) {
        const first_bucket_key_array = Object.keys(ans.keys_contents_left);
        const second_bucket_key_array = Object.keys(second_bucket_keys);
        const second_bucket_key_in_last_pos = second_bucket_key_array[second_bucket_key_array.length - 1];
            ...
            if (second_bucket_curr_obj) {
                ...
                if (etag_pos_on_first_bucket.length === 0) {
                    ({ should_continue, etag_pos_on_first_bucket } = this._etag_not_in_first_bucket(
                        ans, etag_pos_on_first_bucket, version, cur_first_bucket_key, first_bucket_curr_obj, second_bucket_curr_obj));
                ...
                if (etag_pos_on_first_bucket.length === 1) {
                    ...
                    // if all the etags are consecutive, treat them as one with the latest position
                    if (this._is_consecutive(etag_pos_on_first_bucket)) {
                        ...
                        this._process_non_consecutive_etags(ans, etag_pos_on_first_bucket, first_bucket_curr_obj, second_bucket_curr_obj);
                    }
                }
     ...
     */
    _etag_not_in_first_bucket(ans, etag_pos_on_first_bucket, version, cur_first_bucket_key, first_bucket_curr_obj, second_bucket_curr_obj) {
        let pos = 0;
    ...
    /**
     * @param {{ keys_diff_map: any; keys_contents_left?: any; keep_listing_second_bucket?: boolean; }} ans
     * @param {any[]} etag_pos_on_first_bucket
     * @param {any[]} first_bucket_curr_obj
     * @param {any[]} second_bucket_curr_obj
     */
    _process_non_consecutive_etags(ans, etag_pos_on_first_bucket, first_bucket_curr_obj, second_bucket_curr_obj) {
        const first_bucket_etag_last_pos = etag_pos_on_first_bucket.length - 1;
        // get the sliced list of the first bucket
        const first_bucket_sliced_obj = first_bucket_curr_obj.slice(0, etag_pos_on_first_bucket[first_bucket_etag_last_pos] + 1);

        const etag_on_second_bucket = this._get_etag_pos(
            etag_pos_on_first_bucket[0], first_bucket_curr_obj, second_bucket_curr_obj);
