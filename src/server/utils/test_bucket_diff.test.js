...
describe('BucketDiff', () => {
        ...
        describe('get_objects with version', () => {
            ...
            describe('multiple key names', () => {
                ...
                it('list truncated - should return bucket contents minus last key and continuation token as the last returned key name', async () => {
                    ...
                    const result = await bucketDiff.get_objects('bucket-name', '', 100, version, '');
    ...
    describe('_keys_out_of_range', () => {
        ...
        it('should update keys_diff_map , clean keys_contents_left and return true when second bucket list is empty', () => {
            ...
            const stop_compare = bucketDiff._keys_out_of_range(ans, second_bucket_keys);
...
describe('_keys_in_range with version', () => {
    let ans;
    const version = true;
    let bucketDiff;
    ...
    it('case 3: should update keys_diff_map and keys_contents_left when etag appear only once in earlier version', () => {
        ...
        const result = bucketDiff._keys_in_range(ans, second_bucket_keys, second_bucket_cont_token, version);
...
describe('_keys_in_range without version', () => {
    let ans;
    const version = false;
    let bucketDiff;
    let second_bucket_cont_token;
...
describe('BucketDiff aiding functions', () => {
        ...
        describe('_object_grouped_by_key_and_omitted with version', () => {
            ...
            describe('multiple key names', () => {
                ...
                it('list is not truncated with multiple key names, should return all keys grouped into array by key', async () => {
                    ...
                    expected[4] = [{ ETag: 'etag4', Size: 24599, Key: '4', VersionId: 'v4', IsLatest: true, }];
                    const result = await bucketDiff._object_grouped_by_key_and_omitted(list, version);
    ...
    describe('_get_next_key_marker', () => {
        describe('_get_next_key_marker with version', () => {
            it('list is truncated, should return the last key as continuation token', async () => {
                ...
                const result = await bucketDiff._get_next_key_marker(list_objects_response, list, true);
            ...
            it('list is not truncated, should return empty continuation token', async () => {
                ...
                const result = await bucketDiff._get_next_key_marker(list_objects_response, {}, true);
                expect(result).toEqual('');
            });
        });
        describe('_get_next_key_marker without version', () => {
            it('should return continuation token', async () => {
                ...
                const result = await bucketDiff._get_next_key_marker(list_objects_response, {}, false);
    ...
    describe('_is_consecutive', () => {
        it('should return true for consecutive array', () => {
            const array = [1, 2, 3, 4, 5];
            const result = bucketDiff._is_consecutive(array);
            expect(result).toBe(true);
        });

        it('should return false for non-consecutive array', () => {
...
describe('BucketDiff get_keys_diff case 3 with version', () => {
    ...
    it('case 3: etag appear only once in earlier version', async () => {
        ...
        const result = await bucketDiff.get_keys_diff(first_bucket_keys, second_bucket_keys, second_bucket_cont_token, version);
...
describe('BucketDiff get_keys_diff', () => {
    ...
    describe('get_keys_diff _keys_out_of_range flow', () => {
    ...
    describe('get_keys_diff _keys_in_range flow', () => {
