...
const objectid_utils = require('./objectId_utils');
...
objectid_utils.Binary.prototype[util.inspect.custom] = function custom_inspect_binary() {
    return `<objectid_utils.Binary ${this.buffer.toString('base64')} >`;
};
...
function decode_json(schema, val) {
    ...
    if (schema.objectid === true) {
        return new objectid_utils.ObjectId(val);
    }
    if (schema.date === true) {
        return new Date(val);
    }
...
function encode_json(schema, val) {
    ...
    if (schema.objectid === true && val instanceof objectid_utils.ObjectID) {
        return val.toString();
    }
        ...
        try {
            ...
            return res.rows.map(row => { // this is temp fix as all the keys suppose to be ints except _id
                ...
                for (const key of Object.keys(row)) {
                    if (key === '_id') {
                        new_row._id = new objectid_utils.ObjectID(row[key]);
                    } else {
...
class PostgresClient extends EventEmitter {
    ...
    generate_id() {
        return new objectid_utils.ObjectId();
    }
    ...
    resolve_object_ids_recursive(idmap, item) {
        _.each(item, (val, key) => {
            if (val instanceof objectid_utils.ObjectId) {
                if (key !== '_id') {
    ...
    parse_object_id(id_str) {
        return new objectid_utils.ObjectId(String(id_str || undefined));
    }
    ...
    fix_id_type(doc) {
        if (_.isArray(doc)) {
            ...
            doc._id = new objectid_utils.ObjectId(doc._id);
        }
        return doc;
    }

    is_object_id(id) {
        return (id instanceof objectid_utils.ObjectId);
    }
